...
const count = (countTimestamp: number) => {
	...
	if (countDown.value) {
		printVal.value = localStartVal.value - (localStartVal.value - endVal.value) * (progress / localDuration.value)
	} else {
		printVal.value = localStartVal.value + (endVal.value - localStartVal.value) * (progress / localDuration.value)
	}
...
const formatNumber = (num: string | number) => {
	// 格式化小数点后几位
	num = typeof num === 'number' ? `${num.toFixed(props.decimals)}` : `${(+num).toFixed(props.decimals)}`
