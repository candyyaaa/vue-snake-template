...
export default {
  ...
  methods: {
    ...
    render() {
      ...
      window.topologyData.renderer.render(
        window.topologyData.scene,
        window.topologyData.camera
      )
    },
    addMap() {
          ...
          coordinates.forEach((polygonList) => {
            polygonList.forEach((polygon) => {
              ...
              for (let i = 0; i < polygon.length; i++) {
                var pos = this.lngLatToCoord({
                  lng: polygon[i][0],
                  lat: polygon[i][1]
                })
                positions.push(pos.x || 0, 0.01, pos.z || 0)
                i === 0
                  ? shape.moveTo(pos.x || 0, pos.z || 0)
                  : shape.lineTo(pos.x || 0, pos.z || 0)
              }
              ...
              const line = new Line2(
                linGeometry,
                name === '浙江省' ? provinceLineMaterial : cityLineMaterial
              )
